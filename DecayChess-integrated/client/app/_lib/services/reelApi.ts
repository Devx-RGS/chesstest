...
export function useLikeReel() {
    ...
    return useMutation({
        ...
        onMutate: async ({ reelId, action }) => {
            ...
            await queryClient.cancelQueries({ queryKey: ["user-liked-reels"] });
            const prev = queryClient.getQueryData<string[]>(["user-liked-reels"]);
            queryClient.setQueryData<string[]>(["user-liked-reels"], (old) => {
                const current = old || [];
                if (action === "like") return [...current, reelId];
                return current.filter((id) => id !== reelId);
            });
            return { prev };
        },
        onError: (err, variables, context) => {
            console.error("Like mutation failed:", err);
            if (context?.prev) queryClient.setQueryData(["user-liked-reels"], context.prev);
        },
        onSettled: () => {
            queryClient.invalidateQueries({ queryKey: ["reels"] });
            queryClient.invalidateQueries({ queryKey: ["user-liked-reels"] });
        },
    });
}
...
export function useSaveReel() {
    ...
    return useMutation({
        ...
        onMutate: async ({ reelId, action }) => {
            // Cancel in-flight refetches so they don't overwrite our optimistic update
            await queryClient.cancelQueries({ queryKey: ["user-saved-reels"] });
            const prev = queryClient.getQueryData<string[]>(["user-saved-reels"]);
            queryClient.setQueryData<string[]>(["user-saved-reels"], (old) => {
                const current = old || [];
                if (action === "save") return [...current, reelId];
                return current.filter((id) => id !== reelId);
            });
            return { prev };
        },
        onError: (err, variables, context) => {
            ...
            if (context?.prev) queryClient.setQueryData(["user-saved-reels"], context.prev);
        },
        onSettled: () => {
            queryClient.invalidateQueries({ queryKey: ["reels"] });
            queryClient.invalidateQueries({ queryKey: ["user-saved-reels"] });
        },
    });
}
