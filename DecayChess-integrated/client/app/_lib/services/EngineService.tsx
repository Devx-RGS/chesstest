...
export default function EngineService() {
    ...
    useEffect(() => {
        ...
        async function prepareAssets() {
            try {
                const engineDir = `${FileSystem.cacheDirectory}stockfish-engine/`;

                // Create engine directory
                const dirInfo = await FileSystem.getInfoAsync(engineDir);
                if (!dirInfo.exists) {
                ...
                const htmlDest = `${engineDir}stockfish-worker.html`;
                if (htmlAsset.localUri) {
                    const htmlInfo = await FileSystem.getInfoAsync(htmlDest);
                    if (htmlInfo.exists) await FileSystem.deleteAsync(htmlDest);
                    await FileSystem.copyAsync({ from: htmlAsset.localUri, to: htmlDest });
                }

                // --- Download stockfish.js.bin asset and place as stockfish.js ---
                const sfDest = `${engineDir}stockfish.js`;
                const sfInfo = await FileSystem.getInfoAsync(sfDest);
                if (!sfInfo.exists) {
                    ...
                    if (sfAsset.localUri) {
                        await FileSystem.copyAsync({ from: sfAsset.localUri, to: sfDest });
                    }
                }
                ...
                const finalCheck = await FileSystem.getInfoAsync(htmlDest);
                if (finalCheck.exists) {
                    setHtmlUri(htmlDest);
                    const sfCheck = await FileSystem.getInfoAsync(sfDest);
                    console.log(`[EngineService] Assets ready — HTML: ✓, SF: ${sfCheck.exists ? '✓' : '✗'}`);
                } else {
